...
export default function SubscriptionModal({
  ...
  const discountedBase = applyDiscount(baseTokens, coupon);
  const discountedPro = applyDiscount(proTokens, coupon);

  const plans = [
    {
      id: "base",
      label: tSM.planBase,
      price: coupon ? `${discountedBase} ${tSM.perMonth}` : `${baseTokens} ${tSM.perMonth}`,
      originalPrice: coupon ? `${baseTokens} ${tSM.perMonth}` : null,
      tokens: baseTokens,
    ...
    },
    {
      id: "pro",
      label: tSM.planPro,
      price: coupon ? `${discountedPro} ${tSM.perMonth}` : `${proTokens} ${tSM.perMonth}`,
      originalPrice: coupon ? `${proTokens} ${tSM.perMonth}` : null,
      tokens: proTokens,
            ...
            <div key={plan.id} className={`border rounded-2xl p-4 space-y-4 relative ${plan.color}`}>
              ...
              <div>
                ...
                <div className="mt-0.5">
                  ...
                  <p className={`text-lg font-bold ${plan.originalPrice ? "text-chart-3" : ""}`}>{plan.price}</p>
                </div>
              </div>
